...
// botvs@b73a4e940c1c25c49b71b1deb4d1caa7 (test)
...
var state = {};
var orderExchanges = [];
// var timerForBalance = 0;

function onTick() {                  // 主要循环
    
    // var hasPendingOrder = cancelAllOrders();

    if(cancelAllOrders()) state = getExchangesState();

    updateStatePrice();

	if(!hedge()) balanceAccounts();
}
...
function hedge() {
    ...
    // if(sell(maxPair, maxPair.realTicker.Buy - hedgePrice, amount, stripTicker(maxPair.realTicker))) {
    // 	buy(minPair, minPair.realTicker.Sell + hedgePrice, amount, stripTicker(minPair.realTicker));
    // 	calcProfit();
    // 	isBalance = false;
    // 	return true;
    // }
...
function getExchangesState() {
    ...
    var accounts = getExchangeDetails("GetAccount");    
...
function updateStatePrice() {        // 更新 价格
    var now = (new Date()).getTime();     // 记录 当前时间戳
    var tickers = getExchangeDetails("GetTicker");
...
function balanceAccounts() {          // 平衡交易所 账户 钱数 币数
    ...
    if (adjustDiff < state.minStock) {                     // 如果 处理后的 总币差数据 小于 满足所有交易所最小交易量的数据 minStock，即不满足平衡条件
        ...
        // updateStatePrice();                           // 更新 ，并获取 各个交易所行情
...
function buy(detail, price, amount, info){
	var realOrderAmount = amount * (1+(detail.fee.Buy/100));
	if(detail.exchange.Buy(price, realOrderAmount, info)) {
		detail.account.Stocks += amount;
		detail.account.Balance -= price * realOrderAmount;
		orderExchanges[detail.exchange.GetName()] = detail.exchange;
		return true;
	}
	return false;
}
...
function init() {
    ...
    cancelAllOrders(true);                                // 在最开始的时候 不能有任何挂单。所以 会检测所有挂单 ，并取消所有挂单。
    ...
    for (var i = 0; i < initState.details.length; i++) {  // 遍历获取的交易所状态中的 details数组。
        ...
        e.exchange.SetPrecision(2, 5);                            // 设置价格小数位精度为2位, 品种下单量小数位精度为3位
...
//util函数
function getExchangeDetails(type) {
    var result = [];
    while (true) {
        for (var i = 0; i < exchanges.length; i++) {
            if (result[i] == null) {
                // 创建异步操作
                result[i] = exchanges[i].Go(type);
            }
        }
        var failed = 0;
        for (var i = 0; i < exchanges.length; i++) {
            if (typeof(result[i].wait) != "undefined") {
                // 等待结果
                var ret = result[i].wait();
                if (ret) {
                    result[i] = ret;
                    // Log(exchanges[i].GetName(), result[i]);
                } else {
                    // 重试
                    result[i] = null;
                    failed++;
...
function cancelAllOrders(isInit) {                                        // 取消所有订单函数
	var hasPendingOrder = false;
	var exchangesToCheck = typeof(isInit) == 'undefined'? orderExchanges: exchanges;
	
    for (var i in exchangesToCheck) {                    // 遍历交易所对象数组（就是在新建机器人时添加的交易所，对应的对象）
        while (true) {                                              // 遍历中每次进入一个 while 循环
            var orders = null;                                      // 声明一个 orders 变量，用来接收 API 函数 GetOrders  返回的 未完成的订单 数据。
            while (!(orders = exchangesToCheck[i].GetOrders())) {          // 使用 while 循环 检测 API 函数 GetOrders 是否返回了有效的数据（即 如果 GetOrders 返回了null 会一直执行while 循环，并重新检测）
            ...
            for (var j = 0; j < orders.length; j++) {               // 遍历orders  数组， 根据挂出 订单ID，逐个调用 API 函数 CancelOrder 撤销挂单 
                exchangesToCheck[i].CancelOrder(orders[j].Id, orders[j]);
            }
        }
    }
...
function cancelAllOrders_old() {                                        // 取消所有订单函数
	var hasPendingOrder = false;
...
function getProfit(stateInit, stateNow, coinPrice) {                // 获取 当前计算盈亏的函数 
	var allStocks = 0;                                              // 所有的币数
    var allBalance = 0;                                             // 所有的钱数
    var details = stateNow.details;                                               // details 储存详细内容 的数组。
    for (var i = 0; i < details.length; i++) {                    // 遍历 交易所对象数组        
        allStocks += details[i].account.Stocks + details[i].account.FrozenStocks;         // 累计所有 交易所币数
        allBalance += details[i].account.Balance + details[i].account.FrozenBalance;      // 累计所有 交易所钱数        
    }
    stateNow.allStocks = adjustFloat(allStocks);
    stateNow.allBalance = adjustFloat(allBalance);
